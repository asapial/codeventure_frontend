    ...
    <section className="border-b" id="services">
      <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 sm:py-28 lg:px-8">
        <div className="max-w-2xl">
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-blue-600 dark:text-blue-400">What we do</p>
          <h2 className="mt-4 text-balance text-3xl font-bold tracking-[-0.035em] sm:text-5xl">One team for the whole product journey.</h2>
          <p className="mt-5 text-pretty text-lg leading-8 text-muted-foreground">
            The strategic thinking, design craft, and engineering depth to take your product from an open question to a confident launch.
          </p>
        </div>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((service, index) => {
            ...
            return (
              <Card key={service.slug} className="group relative h-full overflow-hidden rounded-2xl border-blue-100/80 bg-card transition-all duration-300 hover:-translate-y-1 hover:border-blue-300 hover:shadow-[0_24px_70px_-32px_rgba(37,99,235,.45)] dark:border-blue-950 dark:hover:border-blue-800">
                ...
                <CardContent className="space-y-5">
                  ...
                  <Link href={`/services/${service.slug}`} className="inline-flex items-center gap-1.5 text-sm font-semibold text-blue-600 transition-colors hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">
                    Explore service <ArrowUpRight className="size-3.5 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" aria-hidden="true" />
                  </Link>
                </CardContent>
              </Card>
            );
